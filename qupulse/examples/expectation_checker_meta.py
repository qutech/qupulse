...
from qupulse.hardware.setup import HardwareSetup, PlaybackChannel, MarkerChannel, MeasurementMask, MetaHardwareSetup
...
class HDAWGAlazar():
    ...
    def __init__(self,
                 ...
                 # setup_names: Tuple[str] = None
                 ):
        
        self._hw_setup = MetaHardwareSetup()
        ...
        self._hdawg = get_hdawg_4x2_multi_setup(self._hw_setup,awg_serial,awg_interface)
        self._init_hdawg()
    ...
    def _init_hdawg(self):
        ...
        for i in range(8):
            ...
            self._hdawg.api_session.setInt(f'/{self._awg_serial}/awgs/{i//2}/synchronization/enable', 1)
...
class ExpectationChecker():
    ...
    CHANNEL_SUBSETS = {f'expcheck_{i}': {f'ZI0_{x}_MARKER_FRONT' for x in y}|{f'ZI0_{x}' for x in y} for i,y in zip(range(4),('AB','CD','EF','GH'))}
    
    def __init__(self,
                 devices: HDAWGAlazar|None,
                 pt: PulseTemplate|None,
                 program_builder: MultiProgramBuilder,
        ...
        if program_builder is not None:
            self._plotting_pb = MultiProgramBuilder.from_mapping(LoopBuilder(),
                                                             channel_subsets=program_builder._channel_subsets)
        else:
            self._plotting_pb = None
    ...
    def _run_aqcuire(self):
        ...
        _, voltages, _ = render_pt_with_times(self._pt,times,{ch for ch in self.PLAY_CHANS},self._plotting_pb)
...
def render_pt_with_times(pt,times,plot_channels,
                         program_builder: MultiProgramBuilder
            ) -> Tuple[np.ndarray, Dict[str, np.ndarray], List[MeasurementWindow]]:
    
    multi_prog = pt.create_program(program_builder=program_builder)
    
    voltages = {}
    
    for sub_loop in multi_prog._flattened_program_map.values():
        waveform, _ = _render_loop(sub_loop, render_measurements=False)
        ...
        sub_voltages = {ch: waveforms._ALLOCATION_FUNCTION(times, **waveforms._ALLOCATION_FUNCTION_KWARGS)
                    for ch in channels if ch in plot_channels}
        
        if set(voltages.keys()).intersection(sub_voltages.keys()):
            raise RuntimeError
            
        
        # print(voltages.keys())
        # print(plot_channels)
        for ch, ch_voltage in sub_voltages.items():
            waveform.get_sampled(channel=ch, sample_times=times, output_array=ch_voltage)
            
        voltages.update(sub_voltages)
...
def get_hdawg_4x2_multi_setup(meta_setup: MetaHardwareSetup, Serial='DEVXXXX', device_interface="USB", address='localhost',idx=None,
                              setup_names: Tuple[str,...] = (f'expcheck_{i}' for i in range(4))
                              ):
    if idx is None:
        idx=str(len(meta_setup.known_awgs))
    ...
    hardware_setups =  [HardwareSetup() for i in range(4)]
    hdawg.channel_grouping = HDAWGChannelGrouping.CHAN_GROUP_4x2
    
    CH_NAMES = 'ABCDEFGH'
    
    
    for awg,setup_id,setup in zip(hdawg.channel_tuples,setup_names,hardware_setups):
        n_channels = awg.num_channels
        awg_channels = CH_NAMES[:n_channels]
        CH_NAMES = CH_NAMES[n_channels:]
        
        meta_setup.add_setup(setup_id, setup)
        
        for ch_i, ch_name in enumerate(awg_channels):
            playback_name = 'ZI'+str(idx)+'_{ch_name}'.format(ch_name=ch_name)
            setup.set_channel(playback_name, PlaybackChannel(awg, ch_i))
            setup.set_channel(playback_name + '_MARKER_FRONT', MarkerChannel(awg, 2 * ch_i))
            # setup.set_channel(playback_name + '_MARKER_BACK', MarkerChannel(awg, 2 * ch_i + 1))
