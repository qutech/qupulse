...
defaultArgs = struct(...
    ...
    'defer_markers',         {plsdata.awg.defaultDeferMarkers}, ... % defer markers to the pulse (ie let qupulse set them)
...
% Drop previously recorded and not measured scanlines
scan.configfn(end+1).fn = @smaconfigwrap;
scan.configfn(end).args = [{@qc.clear_old_scanlines}, ];
...
if ~isempty(a.defer_markers)
    
    if isa(a.defer_markers, 'bool')
        % backwards compability
        if a.defer_markers
            defer_markers = {'AlazarTrig', 'AWGSwitch1', 'AWGSwitch2'};
        else
            defer_markers = {};
        end
    else
        defer_markers = a.defer_markers;
    end
            
    for marker = reshape(defer_markers, 1, [])
        % Set marker to be read from pulse
        scan.configfn(end+1).fn = @smaconfigwrap;
        scan.configfn(end).args = {@smset, marker{1}, Inf};
        
        % .. and restore previous state
        scan.cleanupfn(end+1).fn = @smaconfigwrap;
        scan.cleanupfn(end).args = {@smset,  marker{1}, tune.smget_mat(marker{1})};
    end    
end
