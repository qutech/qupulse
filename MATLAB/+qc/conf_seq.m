...
defaultArgs = struct(...
    ...
    'defer_markers',         true, ...                          % defer markers to the pulse (ie let qupulse set them)
...
if a.defer_markers
    % Set markers to be read from pulse
    scan.configfn(end+1).fn = @smaconfigwrap;
    scan.configfn(end).args = {@smset, 'AlazarTrig', Inf};
    scan.configfn(end+1).fn = @smaconfigwrap;
    scan.configfn(end).args = {@smset, 'AWGSwitch1', Inf};
    scan.configfn(end+1).fn = @smaconfigwrap;
    scan.configfn(end).args = {@smset, 'AWGSwitch2', Inf};
    
    % .. and restore previous state
    scan.cleanupfn(end+1).fn = @smaconfigwrap;
    scan.cleanupfn(end).args = {@smset, 'AlazarTrig', tune.smget_mat('AlazarTrig')};
    scan.cleanupfn(end+1).fn = @smaconfigwrap;
    scan.cleanupfn(end).args = {@smset, 'AWGSwitch1', tune.smget_mat('AWGSwitch1')};
    scan.cleanupfn(end+1).fn = @smaconfigwrap;
    scan.cleanupfn(end).args = {@smset, 'AWGSwitch2', tune.smget_mat('AWGSwitch2')};
end
